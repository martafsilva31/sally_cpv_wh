...
* Computes alices distribution with or without the joint likelihood and joint score as a function of the parameter theta 
* Computes alices training and validition losses

Marta Silva (LIP/IST/CERN-ATLAS), 05/12/2023
"""
...
def augment_test(input_dir,sample_name,nsamples):
  ...
  logging.info(f'sample_name: {sample_name}; nsamples: {nsamples}')
  ...
  folder=f'{input_dir}/testing_samples/alices',
...
def plot_alices_train_val_losses(losses_filename):
    result=np.load(losses_filename)
    arr = result['arr_0']
    fig=plt.figure()


    plt.plot(arr[0],lw=1.5,ls='solid',label=f'Train')
    plt.plot(arr[1],lw=1.5,ls='dashed',label=f'Val')
...
def plot_alices(model_path, args, channel):
    ...
    log_r_hat, _ = alices.evaluate_log_likelihood_ratio(x=x, theta = thetas, test_all_combinations=True)
    fig=plt.figure()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-alices','--plot_alices',help='whether to plot the distribution of the log likelihood evaluated with alices as a function of the parameter theta.',default=False,action='store_true')
    
    parser.add_argument('-joint', '--plot_joint_likelihood',help='whether to plot the joint likelihood ratio and joint score in the alices plot',default=False,action='store_true')
    ...
    for channel in args.channel:
        ...
        if args.plot_inputs:
            ...
            parameter_points=None ,
            filename_bkgonly=f'{args.main_dir}/{channel}_backgroundOnly_noSysts_lhe.h5',
            ...
            log=args.do_log,
            linestyles=None if args.sample_type!='backgroundOnly_noSysts_lhe' else ['dashdot'],
        ...
        if args.augment_test:
            ...
            augment_test(input_dir=args.main_dir,sample_name=f'{channel}_{args.sample_type}',nsamples=args.nsamples)

        if args.plot_alices:
            alices_plot = plot_alices(f"{args.main_dir}/models/{args.alices_observables}/{args.alices_model}/alices_{channel}_{args.sample_type}", args, channel)
        ...
        if args.plot_losses:
            alices_losses=plot_alices_train_val_losses(f'{args.main_dir}/models/{args.alices_observables}/{args.alices_model}/losses_{channel}_{args.sample_type}.npz')
            alices_losses.savefig((f'{args.plot_dir}/alices_losses_{channel}_{args.sample_type}_{args.alices_observables}_{args.alices_model}.pdf'))