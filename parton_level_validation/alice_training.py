...
def train(input_dir,sample_name,nsamples=-1,observable_set='met',training_observables='kinematic_only',model_name=''):
    """  
    ...
    mode: either create only the augmented data samples (mode=augment_only), only do the ALICE method training (mode=train_only), or augment+training (mode=augment_and_train)

    model_name: model name, given to differentiate between, e.g. different ALICE NN configurations
    """
    ...
    result = estimator.train(method='alice',
    ...
    estimator.save(f'{input_dir}/{observable_set}/models/{training_observables}/{model_name}/alice_{sample_name}')
