...
(Same code as sally, but mode in extract limits is "ml")

Marta Silva (LIP/IST/CERN-ATLAS), 25/11/2023
...
if __name__ == "__main__":
  ...
  parser.add_argument('-s','--sample_type',help='sample types to process, without/with samples generated at the BSM benchmark and without/with backgrounds.',choices=['signalOnly_SMonly_noSysts_lhe','withBackgrounds_SMonly_noSysts_lhe','signalOnly_SMonly','signalOnly','withBackgrounds_SMonly','withBackgrounds'],default='signalOnly_SMonly_noSysts_lhe')

  parser.add_argument('-m','--mode',help='what to use to extract the limits, given as input to the expected_limits function',choices=['rate','histo','ml'],required=True)
  ...
  parser.add_argument('-ao','--alices_observables',help='which of the ALICES training observable sets to use when extracting limits from a ALICES-like method',choices=['all_observables_remove_redundant_cos','kinematic_only'],required='ml' in sys.argv)

  parser.add_argument('-am','--alices_model',help='which of the ALICES models (for each of the input variable configurations) to use',required='ml' in sys.argv)
  ...
  parser.add_argument('-bx','--binning_x',help='binning of the variable in the x-axis (can either be a standard observable or output of the ALICES network)',nargs="*",type=float,default=None)
  ...
  parser.add_argument('-nf','--n_fits',help='number of times to shuffle the input dataset and redo the fit',type=int,default=5)
  ...
  if 'ml' in args.mode:
    log_file_path=f"{args.plot_dir}/limits/full_alices_{args.sample_type}_lumi{args.lumi}"  
  else:
  ...
  for i in range(args.n_fits):
    ...
    model_file=f'{args.main_dir}/models/{args.alices_observables}/{args.alices_model}/alices_ensemble_{args.channel}_{args.sample_type}' if 'ml' in args.mode else None,
    ...
    grid_ranges=[(-0.1,0.1)],grid_resolutions=[25])
    ...
    if i%5==0:
      ...
      if args.debug:
        ...
        if 'ml' in args.mode:
          likelihood_histograms.savefig(f'{args.plot_dir}/limits/likelihoods_alices_{args.channel}_{args.sample_type}_{args.alices_observables}_{args.alices_model}_{i}.pdf')
        else:
          if args.observable_y is None:
    ...
    if 'ml' in args.mode:
      llr_histo.savefig(f'{args.plot_dir}/limits/llr_curve_alices_{args.channel}_{args.sample_type}_{args.alices_observables}_lumi{args.lumi}_{i}.pdf')
    else:
      if args.observable_y is None:
    ...
    if 'ml' in args.mode:
      log_file.write(f"{args.alices_observables}, {args.alices_model}, {str(args.binning_x).replace(',',' ')}, {str(central_value[0])}, {str(cl_68).replace(',',' ')}, {str(cl_95).replace(',',' ')} \n") 
    else:
